...
class Addingpolynomialials
{
    static void Main()
    {
        ...
        Console.WriteLine("Subtracting polynomials: {{{0}}}", string.Join(", ", SubtractPolynomials(firstPolynomialMatrix, secondPolynomialMatrix)));
    }
    ...
    static int[] SubtractPolynomials(int[] firstPolynomial, int[] secondPolynomial)
    {
        ...
        else
        {
            ...
            {
                result[i] = firstPolynomial[i] - result[i];
            }
        }
        return result;
    }
